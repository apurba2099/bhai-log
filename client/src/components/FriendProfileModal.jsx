...
export default function FriendProfileModal({ friend, onClose, onChat }) {
  ...
  const status = onlineUsers[friend._id] || {
    isOnline: profile?.isOnline || false,
    lastSeen: profile?.lastSeen,
  };

  //New
  const dp = profile?.profilePic
  ...
  const handleRemoveFriend = async () => {
    ...
    setRemoving(true);
    try {
      await api.delete(`/users/remove-friend/${profile._id}`);
      onClose();
    ...
    <div className="fpm-overlay" onClick={onClose}>
      <div className="fpm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="fpm-dp-wrap" onClick={() => dp && setShowDpFull(true)}>
          {dp ? (
            <img src={dp} alt="dp" className="fpm-dp" />
          ) : (
            <div className="fpm-dp-ph">
        ...
        <div className="fpm-status">
          <span
            className={`fpm-dot ${status.isOnline ? "online" : "offline"}`}
          />
          <span className="fpm-status-text">
            {status.isOnline ? "Online now" : formatLastSeen(status.lastSeen)}
          </span>
        </div>
        ...
        <div className="fpm-actions">
          <button className="pm-btn primary" onClick={onChat}>
            💬 Send Message
          </button>
          <button
            className="pm-btn ghost danger-border"
